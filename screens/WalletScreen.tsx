...
import { View, StyleSheet, Text,ScrollView, FlatList,Image,Pressable, TouchableOpacity, Linking } from "react-native";
...
const Token = [
  ...
  },
  {
    ...
    decimals: 18,
    historicalPricesUsd: { lastDay: [] },
    imageUrl: "https://raw.githubusercontent.com/valora-inc/address-metadata/main/assets/tokens/cKES.png",
  ...
  },
  {
    ...
    decimals: 18,
    historicalPricesUsd: { lastDay: [] },
    imageUrl: "https://raw.githubusercontent.com/valora-inc/address-metadata/main/assets/tokens/cGHS.png",
  ...
  },
  {
    ...
    decimals: 18,
    historicalPricesUsd: { lastDay: [] },
    imageUrl: "https://raw.githubusercontent.com/valora-inc/address-metadata/main/assets/tokens/cZAR.png",
...
export default function WalletScreen(_props: RootStackScreenProps<'CustomWallet'>) {
  const [usdBalance, setUsdBalance] = React.useState<number>(0.0000);
...
React.useEffect(() => {
  ...
  return (
    <View style={style.container}>
          ...
          <Card style={style.card_wallet}>
            ...
            <View style={style.wallet_balance}>
              <View style={style.balance}>
                <Text style={{ color: "#AEC5FF", fontWeight: "500", fontSize: 33, lineHeight: 33, letterSpacing: -1 }}>
                  $
                </Text>
                <Text style={{ color: "#E4EBFE", fontWeight: "500", fontSize: 32, lineHeight: 32, letterSpacing: -2 }}>
      ...
      <DropDownPicker
        ...
        setValue={setValue}
        setItems={setItems}
        textStyle={{fontSize:10,fontWeight:"400",letterSpacing:0,verticalAlign:"bottom"}}
        // zIndex={1000}
        dropDownContainerStyle={{ zIndex: 1000 }}
        style={{ marginBottom: open ? 0 : 0 ,zIndex:1000,height:30,borderColor:"transparent",backgroundColor:"#8DADFE",paddingVertical:0}}
      />
              </View>
            </View>
            ...
            <View style={style.button_wallet}>
              <IconButton style={style.button} textStyle={style.button_text} iconName="send" iconPostion="left" lable="Send"  onPress={onPressSendMoney} />
              <IconButton  style={style.button} textStyle={style.button_text} iconName="arrow-down" iconPostion="left"  lable="Receive" onPress={onPressRecieveMoney} />
              <IconButton style={style.button} textStyle={style.button_text} iconName="swap-horizontal" iconPostion="left"  lable="Swap" onPress={onPressHoldUSD} />
        ...
        <View style={style.wallet_services}>
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={{fontWeight:"400",fontSize:12,letterSpacing:0,verticalAlign:"bottom",color:"#1B1A46"}}>Quick Utilities</Text>
            {/* <Text>Edit</Text> */}
          </View>
      ...
      <View style={style.promotion}>
        <Card style={style.promotion_card} >
          <TouchableOpacity style={style.promotion_logo} onPress={() => Linking.openURL(currentPromo.externalLink)}>
 ...
 <Image  resizeMode="contain"source={require("../assets/ckashxmento.png") } style={{ width: "100%", }} />
 ...
 <Text  style={style.token_balance}>{Number(item.balance).toFixed(4)}</Text>
  <Text style={style.token_symbol}>{item.symbol}</Text>
        </View>
       
        <Text style={style.token_dollar_amount}>${ (Number(item.balance) * Number(item.lastKnownPriceUsd)).toFixed(4) }</Text>
      </View>
        </Card>
      )}
    />
